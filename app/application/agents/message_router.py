...
class MessageRouter:
    def route_orquestrator(self, state):
        ...
        logger.info(f"Roteando (from route_orquestrator) com base no next_step: '{next_step}'")
        ...
        if destination_node:
            logger.info(f"Roteador do orquestrador direcionando para: {destination_node}")
            return destination_node
        else:
            logger.warning(f"Roteador do orquestrador: next_step '{next_step}' não mapeado. Direcionando para fallback_node.")
    ...
    def decide_after_clarification(self, state):
        ...
        logger.info(f"Roteando (from decide_after_clarification) com base no next_step: '{next_step}'")
        ...
        if next_step == "END_AWAITING_USER":
            ...
            logger.warning(f"Valor inesperado para next_step após clarification_node: {next_step}. Direcionando para DEFAULT_END")
            return "DEFAULT_END"
    ...
    def route_after_completeness_check(self, state):
        ...
        """
        next_step = state.get("next_step")
        logger.info(f"Roteando (from route_after_completeness_check) com base no next_step: '{next_step}'")
