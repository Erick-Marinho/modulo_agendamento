...
class Settings(BaseSettings):
    ...
    OPENAI_API_KEY: str = Field(
        ..., env="OPENAI_API_KEY", description="Chave de API do OpenAI"
    )
    ...
    N8N_WEBHOOK_URL: Optional[str] = Field(
        default=None,
        env="N8N_WEBHOOK_URL",
        description="URL do Webhook do N8N para enviar respostas",
    )

    # === LangChain Configuration ===
    LANGCHAIN_TRACING_V2: Optional[bool] = Field(
        default=True, description="Habilitar tracing LangChain"
    )
    LANGSMITH_API_KEY: Optional[str] = Field(
        default=None, description="Chave API LangSmith"
    )
    LANGSMITH_PROJECT: Optional[str] = Field(
        default="modulo_agendamento", description="Projeto LangSmith"
    )
